...
export function ProductDetail(){
    ...
    useEffect(() => {
        async function getProduct() {
            const response = await api.get(`/products/${ id }`) //await espera a requisição
            setProduct(response.data) //retorno que recebemos da api
        }
        getProduct()
    }, [id])
    ...
    return(
        <div>
            <main
            className="w-full max-w-7xl px-4 mx-auto my-6">
                {product && (
